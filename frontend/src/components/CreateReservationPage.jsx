...
function CreateReservationPage() {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            try {
                ...
                setError('Error al cargar la información del vehículo o las sucursales.');
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!vehicle || !pickupBranchId || !returnBranchId || !pickupDate || !returnDate) {
        ...
        try {
            ...
            const pickupDateObject = new Date(startYear, startMonth - 1, startDay, 12, 0, 0); // Month is 0-indexed
            const returnDateObject = new Date(endYear, endMonth - 1, endDay, 12, 0, 0); // Month is 0-indexed
    ...
    if (loading) {
        return <CreateReservationContainer><p>Cargando datos de la reserva...</p></CreateReservationContainer>;
    }

    if (error && !vehicle) { // Only show full error if vehicle details failed to load
        return <CreateReservationContainer><ErrorMessage>{error}</ErrorMessage></CreateReservationContainer>;
    }
