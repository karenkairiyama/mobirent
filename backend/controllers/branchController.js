...
const createBranch = async (req, res) => {
  ...
  if (!name || !address) {
    return res.status(400).json({
      message: "El nombre y la dirección de la sucursal son obligatorios.",
    });
  }
  ...
  try {
    ...
    if (existingBranch) {
      return res
        .status(409)
        .json({ message: "Ya existe una sucursal con ese nombre." });
    }
...
const getAllBranches = async (req, res) => {
  console.log("--- Intentando obtener todas las sucursales ---"); // <-- Añade esto
  try {
    ...
    console.log("Sucursales encontradas:", branches.length, "sucursales."); // <-- Y esto
...
const updateBranch = async (req, res) => {
  ...
  if (!name || !address) {
    return res.status(400).json({
      message: "El nombre y la dirección son obligatorios para actualizar.",
    });
  }
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Sucursal actualizada exitosamente.", branch });
  } catch (error) {
...
const deleteBranch = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Sucursal eliminada exitosamente y vehículos desvinculados.",
    });
  } catch (error) {
